...
reg [`PACKET_WIDTH-1:0] north_packet_in, south_packet_in, east_packet_in, west_packet_in;
reg north_write, south_write, east_write, west_write;
...
always @(posedge clk) begin
    if (local_valid) begin
        local_out_count = local_out_count + 1;
        $display("[t=%0t] LOCAL_OUT event #%0d -> payload = 0x%h", $time, local_out_count, local_packet_out[31:0]);
    end
end

task send_local_packet(input [31:0] payload_marker);
begin
    @(negedge clk);
    // dest=self(0,0,0) src=(0,0,0) type=00 prio=00 payload=marker
    local_packet_in = {2'd0,2'd0,2'd0, 2'd0,2'd0,2'd0, 2'd0,2'd0, payload_marker};
    local_write = 1'b1;
    @(negedge clk);
    local_write = 1'b0;
    local_packet_in = 48'hx;
end
endtask
...
initial begin
    ...
    $display("=== t=%0t : Injecting Packet A (payload=0xAAAA0001) on LOCAL input, dest=self ===", $time);
    send_local_packet(32'hAAAA0001);

    repeat(6) @(negedge clk);

    $display("=== t=%0t : Injecting Packet B (payload=0xBBBB0002) on LOCAL input, dest=self ===", $time);
    send_local_packet(32'hBBBB0002);

    repeat(20) @(negedge clk);

    $display("");
    $display("=== SUMMARY ===");
    $display("Total LOCAL_OUT events observed : %0d", local_out_count);
    $display("Input FIFO (LOCAL port) count    : %0d  (hierarchical peek)", DUT.INPUT_PORT_LOCAL.INPUT_BUFFER.FIFO_INST.count);
    $display("Input FIFO (LOCAL port) rd_ptr    : %0d", DUT.INPUT_PORT_LOCAL.INPUT_BUFFER.FIFO_INST.rd_ptr);
    $display("Input FIFO (LOCAL port) wr_ptr    : %0d", DUT.INPUT_PORT_LOCAL.INPUT_BUFFER.FIFO_INST.wr_ptr);
